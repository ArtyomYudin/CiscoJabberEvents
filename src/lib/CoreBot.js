...
function Botkit(configuration) {
  ...
  function Conversation(task, message) {
    ...
    this.repeat = function () {
      if (this.sent.length) {
        /*
         * pop the last sent message and put it back on the
         * message queue
         */
        let last = this.sent.pop();
        last.sent = false;
      ...
      if (outbound.attachment) {
        ...
        if (outbound.attachment.payload.text && typeof outbound.attachment.payload.text != 'string') {
          outbound.attachment.payload.text = this.replaceTokens(
            outbound.attachment.payload.text[Math.floor(Math.random() * outbound.attachment.payload.text.length)],
          );
        }
  ...
  botkit.setupWebserver = function (port, cb) {
    ...
    botkit.httpserver = botkit.webserver.listen(botkit.config.port, botkit.config.hostname, function () {
