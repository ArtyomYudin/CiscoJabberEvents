...
function JabberBot(configuration) {
  ...
  controller.defineBot(function (botkit, config) {
    ...
    function extractMentionJids(message) {
      let direct_mention_reg = /href="xmpp:\s?(\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+)\s?"/gi;
      ...
      if (stanza.is('message')) {
        if (stanza.attrs.type == 'chat') {
          ...
          if (body) {
            ...
            (xmpp_message.channel = 'chat'), controller.ingest(bot, xmpp_message, null);
          }
          ...
          if (body) {
            ...
            let from_jid = null;
            if (from_split.length > 1) from_jid = from_split[1];
            if (!from_jid) return;

            let history_reg = /xmlns="http:\/\/www.jabber.com\/protocol\/muc#history"/i;
            if (history_reg.test(stanza.toString())) return false;
