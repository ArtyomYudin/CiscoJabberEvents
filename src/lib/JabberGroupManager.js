...
function Caps(node, hash) {
  ...
  if (this.hash !== 'sha-1') {
    throw new Error('Invalid argument provided for hash. Only sha-1 supported');
  }
...
Caps.prototype.generateVersionHash = function () {
  ...
  this.identities.forEach(function (id) {
    str += id.category + '/' + id.type + '/' + (id.lang || '') + '/' + (id.name || '');
    str += '<';
  });
...
function JabberGroupManager(config, xmpp, bot, controller) {
  ...
  function handleRoomDiscoQueryIq(stanza) {
    ...
    if (stanza.attrs.id !== 'room_disco1') return;
    let jid = stanza.attrs.to;
    if (!jid) return;
    let bareJid = jid.split('/')[0];
    if (bareJid.toLowerCase() !== bot.client_jid.toLowerCase()) return;
    ...
    for (let i = 0; i < features.length; i++) {
      ...
      if (feature.attrs.var === 'persistent' || feature.attrs.var === 'muc_persistent') {
  ...
  function saveJoinPersistRooms(room_id, room_password) {
    ...
    controller.storage.teams.get(id, function (err, room) {
      ...
      controller.storage.teams.save(room, function (err) {
        if (err) {
